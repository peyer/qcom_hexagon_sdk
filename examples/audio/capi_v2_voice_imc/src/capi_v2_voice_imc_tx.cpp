...
#define VOICE_IMC_TX_SRC_MAX_BUF_Q_ELEMENTS 1
...
static capi_v2_err_t capi_v2_imc_tx_imc_register_src_dest(imc_src_dst_info_t *imc_src_dst_into, int8_t *key, uint32_t keylen,uint32_t max_bufq_len,uint32_t alloc_num_buf,uint32_t buf_size, imc_src_dst_t src_dst);
...
capi_v2_err_t capi_v2_voice_imc_tx_get_static_properties(capi_v2_proplist_t *init_props_ptr, capi_v2_proplist_t *out_props_ptr)
{
	...
	{
		FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_get_stat_prop(): Error! Received bad property pointer for get property");
	...
	if(init_props_ptr)
	{
		for(i = 0; i < init_props_ptr->props_num; i++)
		{
			...
			default:
				{
					FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_get_stat_prop(): Unsupported static property being set, %d", (int)current_prop_ptr->id);
					break;
				}
			}
		}
	...
	for(i = 0; i < out_props_ptr->props_num; i++)
	{
		...
		case CAPI_V2_INIT_MEMORY_REQUIREMENT:
			{
				if(payload_ptr->max_data_len >= sizeof(capi_v2_init_memory_requirement_t))
				{
					...
					FARF(HIGH, "VOICE_IMC_TX: Property id 0x%lx, param size %lu", (uint32_t)current_prop_ptr->id, payload_ptr->max_data_len);
				}
				else
				{
					FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_get_stat_prop(): Error! Get Property id 0x%lx Bad param size %lu", (uint32_t)current_prop_ptr->id, payload_ptr->max_data_len);
		...
		default:
			{
				FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_get_stat_prop(): Unsupported prop_id 0x%lx", (uint32_t)current_prop_ptr->id);
...
capi_v2_err_t capi_v2_voice_imc_tx_init(capi_v2_t* _pif, capi_v2_proplist_t *init_props_ptr)
{
	...
	if(NULL == _pif)
	{
		FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_init(): Error! received bad pointer");
		return CAPI_V2_EFAILED;
	}
	...
	if(init_props_ptr)
	{
		...
		{
			FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_init(): Error! Set properties failed");
			return CAPI_V2_EFAILED;
		}
	}
	...
	*/
		...
		(NULL != me->capi_v2_imc_tx_imc_info.imc_lib_handle) )
	{
		...
		*/
		...
		FARF(HIGH, "VOICE_IMC_TX: key (%lx) (%lx) (%lx) (%lx)",
		...
		{
			FARF(HIGH, "VOICE_IMC_TX: Failed to register to IMC for Tx Module. result(0x%lx)",imc_result);
			return imc_result;
		}
	}
	else
	{
		FARF(ERROR, "VOICE_IMC_TX: ERROR in fetching IMC handle : service id = (%lx) session id  = (%lx)  IMC lib handle=(%lx) ",
											  me->session_identify.service_id,
											  me->session_identify.session_id,
											  me->capi_v2_imc_tx_imc_info.imc_lib_handle);
	}
...
static capi_v2_err_t capi_v2_imc_tx_process(capi_v2_t* _pif, capi_v2_stream_data_t* input[], capi_v2_stream_data_t* output[])
{
	//FARF(HIGH, "VOICE_IMC_TX: Tx Process has Started !!!");
	if((NULL == _pif) || (NULL== input) || (NULL== output) )
	{
		FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_process(): Received bad property in process");
		return CAPI_V2_EFAILED;
	}
	
	FARF(ERROR, "capi_v2_imc_tx_process: Process");
	...
	for(i=0;i<input[0]->bufs_num;i++)
	{
		...
		{
			std_memscpy(output[0]->buf_ptr[i].data_ptr,output[0]->buf_ptr[i].max_data_len,input[0]->buf_ptr[i].data_ptr,input[0]->buf_ptr[i].actual_data_len);
		}
		...
		(me->capi_v2_imc_tx_imc_info.src_frame_num < VOICE_IMC_RETRY_THRESHOLD))
	{
		...
		FARF(HIGH, "VOICE_IMC_TX: capi_v2_imc_tx_process(): opening communication for inter module communication");
		...
		if(CAPI_V2_FAILED(result))
		{
			FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_process(): Failed to open Communication. result(%lx)",result);
	...
	{
		FARF(FATAL, "VOICE_IMC_TX:  VoiceIMCTx has not opened communication. Returning");
		return CAPI_V2_EOK;
	}
	//
	...
	int32_t *buf = NULL ;
	//FARF(HIGH, "VOICE_IMC_TX:  Trying to pop buffers from the queue");
	...
	if(is_queue_empty == FALSE)
	{
		...
		if( CAPI_V2_FAILED( result))
		{
			FARF(ERROR, "VOICE_IMC_TX:  FAILED to pop buffers from the queue. result(%lx),is_queue_empty(%d)!!",result,(int)is_queue_empty);
			return CAPI_V2_EFAILED;
		}
		...
		*buf = (int32_t)me->counter_imc;
		...
		(me->counter_imc)++;
		FARF(HIGH, "VOICE_IMC_TX: IMC TX sending counter = 0x%X !!",*buf);
		...
		if( CAPI_V2_FAILED(result))
		{
			FARF(ERROR, "VOICE_IMC_TX:  FAILED to push buffers from the queue. result(%lx)!!",result);
			...
			{
				FARF(ERROR, "VOICE_IMC_TX:   Rx handle is not active. Closing the connection");
	...
	{
		FARF(MEDIUM, "VOICE_IMC_TX:  queue is empty. result(%lx)!!",result);
	}
...
static capi_v2_err_t capi_v2_imc_tx_set_param(capi_v2_t* _pif, uint32_t param_id, const capi_v2_port_info_t *port_info_ptr, capi_v2_buf_t *params_ptr)
{
	...
	{
		FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_set_param(): Error! Received bad property pointer for set_param, 0x%lx", param_id);
		return CAPI_V2_EFAILED;
	}
	capi_v2_err_t result = CAPI_V2_EOK;
	capi_v2_imc_tx_t* me = (capi_v2_imc_tx_t*)_pif;
	...
	case CAPI_V2_IMC_Tx_PARAM_ID_ENABLE:
		{
			...
			if (NULL == me->event_cb)
			{
				FARF(ERROR,"VOICE_IMC_TX: Event callback is not set. Unable to raise process event!");
			...
			FARF (HIGH, "Debug : CAPI_V2_EVENT_PROCESS_STATE = %x, me->event_cb = %d", me->event_cb, me->enable);
			result = me->event_cb(me->event_context,CAPI_V2_EVENT_PROCESS_STATE,
					&event_info);
			if (CAPI_V2_FAILED(result))
			{
				FARF(ERROR,
						"VOICE_IMC_TX: Failed to send process_check update event with %lu", result);
			}
	...
	if(new_kpps != me->kpps)
	{
		FARF(HIGH, "VOICE_IMC_TX: capi_v2_imc_tx_set_param(): Voice kpps updated, old %lu, new %lu", me->kpps, new_kpps);
		me->kpps = new_kpps;
		capi_v2_imc_tx_send_kpps(me);
	}
	return result;
...
static capi_v2_err_t capi_v2_imc_tx_get_param(capi_v2_t* _pif, uint32_t param_id, const capi_v2_port_info_t *port_info_ptr, capi_v2_buf_t *params_ptr)
{
	if(NULL == _pif || NULL == params_ptr || NULL == params_ptr->data_ptr)
	{
		FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_get_param(): Error! Received bad property pointer for set_param, 0x%lx", param_id);
		return CAPI_V2_EFAILED;
	}
	capi_v2_imc_tx_t* me = (capi_v2_imc_tx_t*)_pif;
	...
	case VOICE_PARAM_MOD_ENABLE:
		{
			...
			{
				FARF(MEDIUM, "VOICE_IMC_TX: capi_v2_imc_tx_get_param(): IMC TX structure required size = %d, given size = %ld",sizeof(int32_t), pParamsBufferLen);
			...
			*((int32_t*)pParamsBuffer) = 0; // Clearing the whole buffer
            ...
            FARF(HIGH, "VOICE_IMC_TX: enable(%ld)",me->enable);
			if (NULL == me->event_cb)
			{
				FARF(ERROR,"VOICE_IMC_TX: Event callback is not set. Unable to raise process event!");
			...
			//FARF (HIGH, "Debug : Event callback at CAPI_V2_EVENT_PROCESS_STATE = %x", me->event_cb);
			nResult = me->event_cb(me->event_context,CAPI_V2_EVENT_PROCESS_STATE,
					&event_info);
			if (CAPI_V2_FAILED(nResult))
			{
				FARF(ERROR,
						"VOICE_IMC_TX: Failed to send process_check update event with %lu", nResult);
			}
...
static capi_v2_err_t capi_v2_imc_tx_end(capi_v2_t* _pif)
{
	if(NULL == _pif)
	{
		FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_end(): received null pointer in end");
		return CAPI_V2_EFAILED;
	}

	FARF(HIGH, "VOICE_IMC_TX: capi_v2_imc_tx_end(): end()");
...
static capi_v2_err_t capi_v2_imc_tx_set_properties(capi_v2_t* _pif, capi_v2_proplist_t *props_ptr)
{
	...
	if(!_pif || !props_ptr)
	{
		FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_set_properties(): Error! Received bad pointer in set_property");
		return CAPI_V2_EFAILED;
	}
	...
	for(i = 0; i < props_ptr->props_num; i++)
	{
		...
		case CAPI_V2_EVENT_CALLBACK_INFO:
			{
                ...
                FARF(HIGH, "VOICE_IMC_TX: CAPI_V2_EVENT_CALLBACK_INFO is Set  %x", me->event_cb);
				break;
			}
		case CAPI_V2_HEAP_ID:
			{
				...
				{
					FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_set_properties(): Error received bad size(%ld) for heap id property", payload_ptr->actual_data_len);
					return CAPI_V2_EBADPARAM;
				}
				capi_v2_heap_id_t* heap_id_ptr = (capi_v2_heap_id_t*) payload_ptr->data_ptr;
				std_memscpy(&me->heap_id, sizeof(me->heap_id), heap_id_ptr, payload_ptr->actual_data_len);
				break;
			}
		case CAPI_V2_PORT_NUM_INFO:
			{
				...
				{
					FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_set_properties(): Error! Size mismatch for port info property, got %lu", payload_ptr->actual_data_len);
					//bail out
					return CAPI_V2_EBADPARAM;
				}
				...
				{
					FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_set_properties(): Error! invalid number of ports, got num_input_ports(%lu), num_output_ports(%lu)",
				...
				FARF(HIGH, "VOICE_IMC_TX: capi_v2_imc_tx_set_properties(): port number config. num_input_ports(%lu), num_output_ports(%lu)",port_info_ptr->num_input_ports,port_info_ptr->num_output_ports);
				break;
			}
		case CAPI_V2_INPUT_MEDIA_FORMAT:
			{
				...
				{
					FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_set_properties(): Error! input media format is being configured before port_num_info is configured");
					return CAPI_V2_EBADPARAM;
				}
				...
				{
					FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_set_properties(): Error! Size mismatch for input media format property, got %lu", payload_ptr->actual_data_len);
					//bail out
					return CAPI_V2_EBADPARAM;
				}
				...
				for(num_input_ports=0; num_input_ports<me->port_info.num_input_ports; num_input_ports++)
				{
					if(CAPI_V2_FIXED_POINT == pFmt->media_format.format_header.data_format)
					{
						...
						{
							FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_set_properties(): Invalid bits_per_sample(%d) or samping_rate(%lu) or num_channel(%d) for LEC_OUT or NREF output port",
						...
						//copy channel_type
						std_memscpy(&me->input_output_media_format[num_input_ports].data_format.channel_type[0],sizeof(me->input_output_media_format[num_input_ports].data_format.channel_type),
							&pFmt->data_format.channel_type[0],sizeof(pFmt->data_format.channel_type[0])*(pFmt->data_format.num_channels));

					   me->sampling_rate = pFmt->data_format.sampling_rate;;
					   FARF(HIGH, "VOICE_IMC_TX: capi_v2_imc_tx_set_properties(): set_props bits_per_sample(%lu), samping_rate(%lu), num_channels(%lu)",
					          pFmt->data_format.bits_per_sample, pFmt->data_format.sampling_rate, pFmt->data_format.num_channels);
					}
					else
					{
						FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_set_properties(): Invalid media fmt(%ul)",pFmt->media_format.format_header.data_format);
						return CAPI_V2_EBADPARAM;
					}
				   ...
				   process_event_info.payload.actual_data_len = sizeof(me->input_output_media_format[0])*(me->port_info.num_output_ports);
				   process_event_info.payload.data_ptr = (int8_t *) &me->input_output_media_format;
				   process_event_info.payload.max_data_len = sizeof(me->input_output_media_format);
				   ///FARF (HIGH, "VOICE_IMC_TX: Debug: Event callback CAPI_V2_EVENT_OUTPUT_MEDIA_FORMAT_UPDATED = %x", me->event_cb);
		...
		case CAPI_V2_OUTPUT_MEDIA_FORMAT:
		{
            ...
            FARF(HIGH,"VOICE_IMC_TX: Received output media fmt");
            break;
		}
		...
		case CAPI_V2_SESSION_IDENTIFIER:
			{
				...
				{
					FARF(ERROR, "VOICE_IMC_TX:  Error! Size mismatch for port info property, got %lu", payload_ptr->actual_data_len);
					//bail out
					return CAPI_V2_EBADPARAM;
				}
				...
				FARF(HIGH, "VOICE_IMC_TX:  session_identifies service_id(%lx), session_id(%lx)",
					me->session_identify.service_id,
					me->session_identify.session_id);
				break;
			}
		default:
			{
				FARF(HIGH, "VOICE_IMC_TX: capi_v2_imc_tx_set_properties(): Unsupported property being set %d, ignoring", (int)current_prop_ptr->id);
				break;
			}
		}
	}
...
static capi_v2_err_t capi_v2_imc_tx_get_properties(capi_v2_t* _pif, capi_v2_proplist_t *props_ptr)
{
	if(!_pif || !props_ptr)
	{
		FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_get_properties(): Error! Received bad pointer in get_property");
		return CAPI_V2_EFAILED;
	}
		...
		default:
			{
				...
				FARF(HIGH, "VOICE_IMC_TX: capi_v2_imc_tx_get_properties(): Unsupported prop ID %d", (int)current_prop_ptr->id);
				break;
			}
		}
	}
...
static capi_v2_err_t capi_v2_imc_tx_imc_register_src_dest(imc_src_dst_info_t *imc_src_dst_into, int8_t *key, uint32_t keylen,uint32_t max_bufq_len,uint32_t alloc_num_buf,uint32 buf_size, imc_src_dst_t src_dst)
{
	...
	FARF(HIGH, "VOICE_IMC_TX: capi_v2_imc_tx_imc_register_src_dest: module is trying to register for IMC CommID(%#lx), src_dst(%d)",comm_id,(int)src_dst);

	//Check if the key is already received
	if(IMC_STATE_NONE != imc_src_dst_into->state)
	{
		FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_imc_register_src_dest: Error!! trying to set multiple Key values for the same CommID(%lx). state(%d)",
	...
	{
		FARF(HIGH, "VOICE_IMC_TX: imc_src_dst_into->imc_lib_handle->vtable->src_register (%x)", imc_src_dst_into->imc_lib_handle->vtable->src_register);
		imc_result = imc_src_dst_into->imc_lib_handle->vtable->src_register(&imc_src_dst_into->source_handle,max_bufq_len);
		if(CAPI_V2_FAILED(imc_result))
		{
			FARF(HIGH, "VOICE_IMC_TX: capi_v2_imc_tx_imc_register_src_dest: Error!! source failed to register, result(0x%lx)",imc_result);
			return imc_result;
		}
		...
		if(CAPI_V2_FAILED(imc_result))
		{
			FARF(HIGH, "VOICE_IMC_TX: capi_v2_imc_tx_imc_register_src_dest: Error!! source failed to push buffers, result(0x%lx)",imc_result);
			return imc_result;
		}
		else
		{
	...
	else if(IMC_DESTINATION == src_dst)
	{
		...
		if(CAPI_V2_FAILED(imc_result))
		{
			FARF(HIGH, "VOICE_IMC_TX: capi_v2_imc_tx_imc_register_src_dest: Error!! destination failed to register, result(0x%lx)",imc_result);
			return imc_result;
		}
		else
		{
	...
	{
		FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_imc_register_src_dest: Error!! invalid source/destination param(%d)",(int)src_dst);
...
static uint32_t capi_v2_imc_tx_get_kpps(capi_v2_imc_tx_t* me)
{
	...
	FARF(HIGH, "In capi_v2_imc_tx_get_kpps: me->sampling_rate = %d", me->sampling_rate);
	if (TRUE == me->enable)
	{
...
static void capi_v2_imc_tx_send_kpps(capi_v2_imc_tx_t * me)
{
	...
	if(me->event_cb)
	{
		...
		if(result)
		{
			FARF(ERROR, "VOICE_IMC_TX: capi_v2_imc_tx_send_kpps(): Error! Voice capi_v2_imc_tx kpps reporting failed, error %ld", result);
		}
	}
	return;
}
...
static void capi_v2_imc_tx_get_imc_handle(capi_v2_imc_tx_t* me)
{
	...
	if(CAPI_V2_FAILED(result))
	{
		FARF(ERROR, "VOICE_IMC_TX: get_imc_handle(): failed to get IMC lib handle. result(0x%lx)",result);
		return;
	}
	else
	{
		FARF(HIGH, "VOICE_IMC_TX: get_imc_handle(): VOICE IMC Tx received IMC lib handle");
	}
