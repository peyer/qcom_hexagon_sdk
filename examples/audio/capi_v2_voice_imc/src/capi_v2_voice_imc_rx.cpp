...
/* Delay values*/
/* worst case profiled values */
#define VOICE_IMC_NB_DELAY_IN_US  (110)
#define VOICE_IMC_WB_DELAY_IN_US  (210)
#define VOICE_IMC_FB_DELAY_IN_US  (310)
...
capi_v2_err_t capi_v2_voice_imc_rx_get_static_properties(capi_v2_proplist_t *init_props_ptr, capi_v2_proplist_t *out_props_ptr)
{
	...
	{
		FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_get_stat_prop(): Error! Received bad property pointer for get property");
	...
	if(init_props_ptr)
	{
		for(i = 0; i < init_props_ptr->props_num; i++)
		{
			...
			default:
				{
					FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_get_stat_prop(): Unsupported static property being set, %d", (int)current_prop_ptr->id);
					break;
				}
			}
		}
	...
	for(i = 0; i < out_props_ptr->props_num; i++)
	{
		...
		case CAPI_V2_INIT_MEMORY_REQUIREMENT:
			{
				...
				{
					FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_get_stat_prop(): Error! Get Property id 0x%lx Bad param size %lu", (uint32_t)current_prop_ptr->id, payload_ptr->max_data_len);
		...
		default:
			{
				FARF(HIGH, "VOICE_IMC_RX: capi_v2_voice_imc_rx_get_stat_prop(): Unsupported prop_id 0x%lx", (uint32_t)current_prop_ptr->id);
...
capi_v2_err_t capi_v2_voice_imc_rx_init(capi_v2_t* _pif, capi_v2_proplist_t *init_props_ptr)
{
	...
	if(NULL == _pif)
	{
		FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_init(): Error! received bad pointer");
		return CAPI_V2_EFAILED;
	}

	capi_v2_voice_imc_rx_t* me = (capi_v2_voice_imc_rx_t*)_pif;
	memset(me,0,sizeof(capi_v2_voice_imc_rx_t));
	...
	if(init_props_ptr)
	{
		...
		{
			FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_init(): Error! Set properties failed");
			return result;
		}
	}
		...
		(NULL != me->capi_v2_voice_imc_rx_imc_info.imc_lib_handle) )
	{
		...
		FARF(HIGH, "VOICE_IMC_RX: imc_set_param(): key (%lx) (%lx) (%lx) (%lx)",
		...
		{
			FARF(HIGH, "VOICE_IMC_RX: Failed to register to IMC for Rx Module. result(0x%lx)",imc_result);
			return imc_result;
		}
	}
	else
	{
		FARF(ERROR, "VOICE_IMC_RX: ERROR in fetching IMC handle : service id = (%lx) session id  = (%lx)  IMC lib handle=(%lx) ",
...
static capi_v2_err_t capi_v2_voice_imc_rx_process(capi_v2_t* _pif, capi_v2_stream_data_t* input[], capi_v2_stream_data_t* output[])
{
	//FARF(HIGH, "VOICE_IMC_RX: Rx Process has Started !!!");
	if((NULL == _pif) || (NULL== input) || (NULL== output) || (input[0]->bufs_num != 1))
	{
		FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_process(): Received NULL pointers in process");
		return CAPI_V2_EFAILED;
	}
	
	FARF(ERROR, "capi_v2_voice_imc_rx_process: Process");
	capi_v2_err_t result=CAPI_V2_EOK;
...
static capi_v2_err_t capi_v2_voice_imc_rx_end(capi_v2_t* _pif)
{
	if(NULL == _pif)
	{
		FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_end(): Error! Received bad pointer");
		return CAPI_V2_EFAILED;
	}
	capi_v2_voice_imc_rx_t* me = (capi_v2_voice_imc_rx_t*) _pif;

	FARF(HIGH, "VOICE_IMC_RX: capi_v2_voice_imc_rx_end(): end()");
...
static capi_v2_err_t capi_v2_voice_imc_rx_set_param(capi_v2_t* _pif, uint32_t param_id, const capi_v2_port_info_t *port_info_ptr, capi_v2_buf_t *params_ptr)
{
	...
	{
		FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_set_param(): Error! Received bad property pointer for set_param, 0x%lx", param_id);
		return CAPI_V2_EFAILED;
	}
	capi_v2_err_t result = CAPI_V2_EOK;
	capi_v2_voice_imc_rx_t* me = (capi_v2_voice_imc_rx_t*)_pif;
	...
	case CAPI_V2_IMC_Rx_PARAM_ID_ENABLE:
		{
            ...
            FARF(HIGH, "VOICE_IMC_RX: enable(%ld)",me->enable);


			if (NULL == me->event_cb)
			{
				FARF(ERROR,"VOICE_IMC_RX: Event callback is not set. Unable to raise process event!");
			...
			{
				FARF(ERROR,
						"VOICE_IMC_RX: Failed to send process_check update event with %lu", result);
			}
	...
	if(new_kpps != me->kpps)
	{
		FARF(HIGH, "VOICE_IMC_RX: capi_v2_voice_imc_rx_set_param(): Voice voice_imc_rx kpps updated, old %lu, new %lu", me->kpps, new_kpps);
		me->kpps = new_kpps;
	...
	{
		FARF(HIGH, "VOICE_IMC_RX: capi_v2_voice_imc_rx_set_param(): Voice voice_imc_rx delay updated, old %lu, new %lu", me->delay, new_delay);
		me->delay = new_delay;
...
static capi_v2_err_t capi_v2_voice_imc_rx_get_param(capi_v2_t* _pif, uint32_t param_id, const capi_v2_port_info_t *port_info_ptr, capi_v2_buf_t *params_ptr)
{
	if(NULL == _pif || NULL == params_ptr || NULL == params_ptr->data_ptr)
	{
		FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_get_param(): Error! Received bad property pointer for set_param, 0x%lu", param_id);
		return CAPI_V2_EFAILED;
	}
	...
	case VOICE_PARAM_MOD_ENABLE:
		{
			...
			{
				FARF(MEDIUM, "VOICE_IMC_RX: capi_v2_imc_tx_get_param(): structure required size = %d, given size = %ld",sizeof(int32_t), pParamsBufferLen);
			...
			else
		 {
			...
			{
				FARF(ERROR,
						"VOICE_IMC_RX: Failed to send process_check update event with %lu", nResult);
			}
...
static capi_v2_err_t capi_v2_voice_imc_rx_set_properties(capi_v2_t* _pif, capi_v2_proplist_t *props_ptr)
{
	...
	if(!_pif || !props_ptr)
	{
		FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_set_properties(): Error! Received bad pointer in set_property");
		return CAPI_V2_EFAILED;
	}
	...
	for(i = 0; i < props_ptr->props_num; i++)
	{
		...
		case CAPI_V2_EVENT_CALLBACK_INFO:
			{
                ...
                FARF(HIGH, "VOICE_IMC_RX: CAPI_V2_EVENT_CALLBACK_INFO property is Set ");
		...
		case CAPI_V2_HEAP_ID:
			{
				...
				{
					FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_set_properties(): Error received bad size(%ld) for heap id property", payload_ptr->actual_data_len);
					return CAPI_V2_EBADPARAM;
				}
		...
		case CAPI_V2_PORT_NUM_INFO:
			{
				...
				{
					FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_set_properties: received bad size(%ld) for port_num_info property",
				...
				{
					FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_set_properties: received unsupported in_num_ports(%ld) or out_num_ports(%ld)",
		...
		case CAPI_V2_INPUT_MEDIA_FORMAT:
			{
				...
				{
					FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_set_properties(): Error! Size mismatch for input format property, got %lu", payload_ptr->actual_data_len);
					//bail out
					return CAPI_V2_EBADPARAM;
				}
				...
				if(CAPI_V2_FIXED_POINT == pFmt->format_header.data_format)
				{
					...
					{
						FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_set_properties(): Invalid bits_per_sample(%lu) or samping_rate(%lu) or num_channels(%lu)",pStd->bits_per_sample,pStd->sampling_rate,pStd->num_channels);
						return CAPI_V2_EBADPARAM;
					}
					me->sampling_rate = pStd->sampling_rate;
					FARF(HIGH, "VOICE_IMC_RX: capi_v2_voice_imc_rx_set_properties(): set_props bits_per_sample(%lu), samping_rate(%lu), num_channel(%lu)",pStd->bits_per_sample,pStd->sampling_rate,pStd->num_channels);
				}
				else
				{
					FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_set_properties(): Invalid media fmt(%d)",(int)pFmt->format_header.data_format);
					return CAPI_V2_EBADPARAM;
				}
				break;
			}
		...
		{
            FARF(HIGH,"VOICE_IMC_RX: Received output media fmt");
            break;
		}
		...
		case CAPI_V2_SESSION_IDENTIFIER:
			{
				...
				{
					FARF(ERROR, "VOICE_IMC_RX: Error! Size mismatch for port info property, got %lu", payload_ptr->actual_data_len);
					//bail out
					return CAPI_V2_EBADPARAM;
				}
				...
				FARF(HIGH, "VOICE_IMC_RX: capi_v2_voice_imc_rx_set_properties(): session_identifies service_id(%lx), session_id(%lx)",
					me->session_identify.service_id,
					me->session_identify.session_id);
				break;
			}
		default:
			{
				FARF(HIGH, "VOICE_IMC_RX: capi_v2_voice_imc_rx_set_properties(): Unsupported property being set %d, ignoring", (int)current_prop_ptr->id);
				break;
			}
		}
	}
...
static capi_v2_err_t capi_v2_voice_imc_rx_get_properties(capi_v2_t* _pif, capi_v2_proplist_t *props_ptr)
{
	if(!_pif || !props_ptr)
	{
		FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_get_properties(): Error! Received bad pointer in get_property");
		return CAPI_V2_EFAILED;
	}
		...
		default:
			{
				...
				FARF(HIGH, "VOICE_IMC_RX: capi_v2_voice_imc_rx_get_properties(): Unsupported prop ID %d", (int)current_prop_ptr->id);
				break;
			}
		}
	}
...
static void voice_imc_rx_recieve_buf(capi_v2_voice_imc_rx_t* me)
{
	...
	int32_t *buf = NULL ;
	//FARF(HIGH, "VOICE_IMC_RX: Begin voice_imc_rx_recieve_buf()");
	...
	do
	{
		...
		///FARF(HIGH, "VOICE_IMC_RX: imc_result %d, is_queue_empty %d", imc_result, is_queue_empty);
		if(is_queue_empty)
		{
			FARF(HIGH, "VOICE_IMC_RX: capi_v2_voice_imc_rx_get_stat_prop(): Queue is empty");
			return;
		}
		else if(CAPI_V2_FAILED(imc_result))
		{
			//FARF(HIGH, "VOICE_IMC_RX: voice_imc_rx_recieve_buf(): error in popping imc payload, result(0x%lx)",imc_result);
			return;
		}
		else
		{
			FARF(HIGH, "VOICE_IMC_RX: IMC RX recieved counter = 0x%X !!",(int32_t)*buf);
			imc_result = imc_handle->imc_lib_handle->vtable->dest_return_buf(imc_handle->dest_handle,buf,result);
			if(CAPI_V2_FAILED(imc_result))
			{
				FARF(ERROR, "VOICE_IMC_RX: voice_imc_rx_recieve_buf(): error in pushing imc payload. result(%lx)",imc_result);
				return;
			}
		}
...
static void capi_v2_voice_imc_rx_send_kpps(capi_v2_voice_imc_rx_t * me)
{
	...
	if(me->event_cb)
	{
		...
		if(result)
		{
			FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_send_kpps(): Error! Voice kpps reporting failed, error %ld", result);
		}
	}
	return;
}
...
static void capi_v2_voice_imc_rx_send_delay(capi_v2_voice_imc_rx_t * me)
{
	...
	if(me->event_cb)
	{
		...
		if(result)
		{
			FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_rx_send_delay(): Error! Voice delay reporting failed, error %ld", result);
		}
	}
	return;
}
...
static void capi_v2_voice_imc_rx_get_imc_handle(capi_v2_voice_imc_rx_t* me)
{
	...
	if(CAPI_V2_FAILED(result))
	{
		FARF(ERROR, "VOICE_IMC_RX: get_imc_handle(): failed to get IMC lib handle. result(0x%lx)",result);
		return;
	}
	else
	{
		FARF(HIGH, "VOICE_IMC_RX: get_imc_handle(): IMC Rx received IMC lib handle");
	}
...
static capi_v2_err_t capi_v2_voice_imc_register_src_dest(imc_src_dst_info_t *imc_src_dst_into, int8_t *key, uint32_t keylen,uint32_t max_bufq_len,uint32_t alloc_num_buf,uint32 buf_size, imc_src_dst_t src_dst)
{
	...
	FARF(HIGH, "VOICE_IMC_RX: capi_v2_voice_imc_register_src_dest: module is trying to register for IMC CommID(%#lx), src_dst(%d)",comm_id,(int)src_dst);

	//Check if the key is already received
	if(IMC_STATE_NONE != imc_src_dst_into->state)
	{
		FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_register_src_dest: Error!! trying to set multiple Key values for the same CommID(%lx). state(%d)",
	...
	if(IMC_SOURCE == src_dst)
	{
		...
		if(CAPI_V2_FAILED(imc_result))
		{
			FARF(HIGH, "VOICE_IMC_RX: capi_v2_voice_imc_register_src_dest: Error!! source failed to register, result(0x%lx)",imc_result);
			return imc_result;
		}
		...
		if(CAPI_V2_FAILED(imc_result))
		{
			FARF(HIGH, "VOICE_IMC_RX: capi_v2_voice_imc_register_src_dest: Error!! source failed to push buffers, result(0x%lx)",imc_result);
			return imc_result;
		}
		else
		{
	...
	else if(IMC_DESTINATION == src_dst)
	{
		...
		if(CAPI_V2_FAILED(imc_result))
		{
			FARF(HIGH, "VOICE_IMC_RX: capi_v2_voice_imc_register_src_dest: Error!! destination failed to register, result(0x%lx)",imc_result);
			return imc_result;
		}
		else
		{
			imc_src_dst_into->state =  IMC_STATE_REGISTERED;
			FARF(HIGH, "VOICE_IMC_RX: Rx Module Registered: %d", imc_src_dst_into->state);
		}
	}
	else
	{
		FARF(ERROR, "VOICE_IMC_RX: capi_v2_voice_imc_register_src_dest: Error!! invalid source/destination param(%d)",(int)src_dst);
