...
static void capi_v2_gain_raise_output_media_format_event(capi_v2_gain_t *me_ptr)
{
	...
	{
		FARF(ERROR,"CAPIv2 gain: Failed to send output media format updated event with %lu",
				capi_v2_result);
	}
    else
    {
        FARF(HIGH,"CAPI V2 GAIN: Success: send output media format updated event with %lu", capi_v2_result);
    }
}
