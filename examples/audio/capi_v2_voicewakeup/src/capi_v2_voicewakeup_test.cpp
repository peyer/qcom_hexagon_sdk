		...
		capi_v2_proplist_t* static_properties)
{
	void* h = 0;
	//int (*pfn)(void) = 0;
	const char* cpszmodname = "capi_v2_voicewakeup.so";
	const char* cpszget_static_properties = "capi_v2_voicewakeup_get_static_properties";
	const char* cpszinit = "capi_v2_voicewakeup_init";
	...
	TRY(err, test_capi_v2_main(get_static_properties_f,
					...
					"../data/input.raw",
					(const char*)NULL,
					"../data/enable_keyword_detection.cfg"));
#endif
...
#ifdef ENABLE_COMMAND_LINE_PARAMS
	FARF(HIGH,
			"       test_capi_v2_main(inp_args)                                    ");
	TRY(err,
			test_capi_v2_main(capi_v2_voicewakeup_get_static_properties,
					capi_v2_voicewakeup_init,
					init_set_properties,
					static_properties,
					input_args->input_filename,
					input_args->output_filename,
					input_args->config_filename));
#else
	FARF(HIGH, "       test_capi_v2_main(hardCodedStrs)                                    ");
	TRY(err, test_capi_v2_main(capi_v2_voicewakeup_get_static_properties,
					capi_v2_voicewakeup_init,
					init_set_properties,
					static_properties,
					"../data/input.raw",
...
void get_eargs(int32_t argc, char* argv[], args_t* input_args) {
	...
	if (argc < 5) {
		...
		while ((input_option = getopt(argc, argv, "i:c:")) != -1) {
			switch(input_option) {
			case 'i':
...
int test_main_start(int argc, char* argv[]) {
	...
	memset(input_args, 0, sizeof(args_t));
	...
	// VoiceWakeup example doesn't set nor get properties
