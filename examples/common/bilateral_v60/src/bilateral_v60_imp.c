...
AEEResult bilateral_v60_bilateral9x9(const uint8* src, int srcLen, int32 stride, int32 width, 
    int32 height, uint8* dst, int dstLen, int32 *dspUsec) 
{
    *dspUsec = 0;
// only supporting HVX version in this example.
    ...
    *dspUsec = (int32)(endTime - startTime);
    FARF(HIGH,"bilateral profiling: %d PCycles, %d microseconds. Observed clock rate %d MHz",
