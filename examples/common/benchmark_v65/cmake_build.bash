...
# Test on the Device
    ...
    app=benchmark_device
    ANDROID_BUILD=android_Release_aarch64.cmake
    adb push ${ANDROID_BUILD}/${app} /vendor/bin
    adb push ${ANDROID_BUILD}/libbenchmark.so /vendor/lib64/
    adb push ${HEXAGON_BUILD}/libbenchmark_skel.so /vendor/lib/rfsa/adsp/
    adb push ${HEXAGON_BUILD}/libdspCV_skel.so /vendor/lib/rfsa/adsp/
    adb shell chmod 777 /vendor/bin/${app}
    adb shell " /vendor/bin/${app} -o /data/local/tmp/benchmark.csv -f bilateral -P \"1\" -L 10 -l 10 -s"
}
...
then
    ...
    if [ "$2" = "qurt"];        
    then
        doHexagon hexagon_ReleaseG_dynamic_toolv83_v66 benchmark_skel qurt    
    else
        doHexagon hexagon_ReleaseG_dynamic_toolv83_v66 benchmark_skel
    fi    
    cd ${_src} && \
       ${CMAKE} \
       --build \
       ${_build} \
       --target benchmark_skel -- \
       && \
       cd -
elif [ "to$1" = "toclean" ]; 
then
...
then
    ...
    if [ "$1" = "qurt" ];        
    then
        doHexagon hexagon_ReleaseG_dynamic_toolv83_v66 benchmark_q-sim qurt    
    else
        doHexagon hexagon_ReleaseG_dynamic_toolv83_v66 benchmark_q-sim
    fi    
    cd ${_src} && \
       ${CMAKE} \
       --build \
       ${_build} \
       --target benchmark_q-sim -- \
       && \
       cd -
