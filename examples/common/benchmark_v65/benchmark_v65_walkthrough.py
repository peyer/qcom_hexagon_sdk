...
example_name = "benchmark_v65"
unsupported_target_info = ["8998","8996","sdm660"]
target_info = target_list()
benchmark_focus_list = ["epsilon", "bilateral", "fast9", "integrate", "dilate3x3", "dilate5x5", "conv3x3", "gaussian7x7", "sobel3x3", "fft", "scatter", "gather"]

#******************************************************************************
# Parser for cmd line options
#******************************************************************************
parser = argparse.ArgumentParser(prog='benchmark_v65_walkthrough.py', description=__doc__, formatter_class=RawTextHelpFormatter)
call_parser(parser)
...
def run_example():
	...
	hex_variant = hex_variant.replace("Debug","ReleaseG")

	example_name_exe=HEXAGON_SDK_ROOT +EXAMPLE_PATH+example_name+'/'+variant+'/ship/benchmark'
	libexample_name=HEXAGON_SDK_ROOT +EXAMPLE_PATH+example_name+'/'+variant+'/ship/libbenchmark.so'
	libexample_name_skel=HEXAGON_SDK_ROOT +EXAMPLE_PATH+example_name+'/'+hex_variant+'/ship/libbenchmark_skel.so'
	...
	FILE_NAME = "/data/local/benchmark.csv"
	...
	if os.path.exists(FILE_NAME):
		print_and_run_cmd('adb -s '+device_number+'  wait-for-device shell rm '+FILE_NAME)
	...
	print_and_run_cmd('adb -s '+device_number+'  wait-for-device shell chmod 777 '+APPS_DST+'/benchmark')
	...
	for focus in benchmark_focus_list:
		for pwr_level in [0,1,6]:
			print_and_run_cmd('adb -s '+device_number+'  wait-for-device shell export LD_LIBRARY_PATH='+LIB_DST+':$LD_LIBRARY_PATH '+DSP_LIB_SEARCH_PATH+'='+DSP_LIB_PATH+' '+APPS_DST+'/benchmark -o '+FILE_NAME+' -f '+focus+ ' -P '+str(pwr_level)+ ' -L 10 -l 10 -s')

	print "Saving benchmark results to "+FILE_NAME
	print_and_run_cmd('adb -s '+device_number+'  wait-for-device pull '+FILE_NAME+' .')
	print "Done"
