...
benchmark_focus_list = ["epsilon", "bilateral", "fast9", "integrate", "dilate3x3", "dilate5x5", "conv3x3", "gaussian7x7", "sobel3x3"]

#******************************************************************************
# Parser for cmd line options
#******************************************************************************
parser = argparse.ArgumentParser(prog='benchmark_walkthrough.py', description=__doc__, formatter_class=RawTextHelpFormatter)
call_parser(parser)
...
def run_example():
	...
	for focus in benchmark_focus_list:
		for pwr_level in [0,1,6]:
			print_and_run_cmd('adb -s '+device_number+'  wait-for-device shell export LD_LIBRARY_PATH='+LIB_DST+':$LD_LIBRARY_PATH '+DSP_LIB_SEARCH_PATH+'='+DSP_LIB_PATH+' '+APPS_DST+'/'+example_name+' -o '+FILE_NAME+' -f '+focus+ ' -p '+str(pwr_level)+ ' -L 10 -l 10 -s')
