...
static void dilate3x3_ref (unsigned char *src, int srcStride, int width, int height, unsigned char *dst, int dstStride)
{
  int x, y;
  unsigned char max;
...
int test_main_start(int argc, char* argv[])
{
    ...
    if (argc == 1) 
    {
        printf("Usage: dilate3x3_v60 <width (must be multiple of 128)> <height> <number of frames> <uSec per frame> <DSP MIPS vote> <BUS MBPS vote> <verbose (0/1)>\n");
    ...
    // call API
	printf( "calling dilate3x3_v60 on a %dx%d image...\n", (int)width, (int)height);
    ...
    for (i = 0; i < LOOPS; i++)
    {
        ...
        retVal = dilate3x3_v60_dilate(src, size, stride, width, height, dst, size, stride, (int32*)&dspUsec);
    ...
    VERIFY(0 == retVal);
#ifdef __hexagon__
    printf("run time of dilate3x3_v60: %llu PCycles for %d iterations\n", totalTime, LOOPS);
#else
	printf("run time of dilate3x3_v60: %llu microseconds for %d iterations. Last iteration DSP-measured time: %d uSec\n", totalTime, LOOPS, dspUsec);
#endif
	printf("return value from dilate3x3_v60: %d \n", retVal);
