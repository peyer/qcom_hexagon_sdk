...
AEEResult dilate3x3_v60_dilate(
    const uint8* imgSrc, 
    int inpLen, 
    int32 srcStride, 
    int32 srcWidth, 
...
// only supporting HVX version in this example.
    ...
    *dspUsec = (int)(endTime - startTime);
    FARF(HIGH,"dilate3x3_v60 profiling: %d PCycles, %d microseconds. Observed clock rate %d MHz",
