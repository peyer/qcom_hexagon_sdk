...
@file
   conv3x3a16_v60_imp.cpp

@brief
   implementation file for convolution 3x3 filter RPC interface.
...
} conv3x3a16_callback_t;

/*===========================================================================
    LOCAL FUNCTION
===========================================================================*/
// multi-threading callback function
static void conv3x3a16_callback(
    void* data
)
{
    conv3x3a16_callback_t    *dptr = (conv3x3a16_callback_t*)data;
    signed char *mask = (signed char *)dptr->mask;
    int shift         = dptr->shift;
    ...
    for (i = 0; i < length; i+=2)
    {
        ...
        conv3x3Per2Row(src, dptr->srcStride, dptr->srcWidth, mask, shift, dst, dptr->dstStride);
...
AEEResult conv3x3a16_v60_conv3x3(
    const uint8* imgSrc, 
    int inpLen, 
    int32 srcStride, 
    int32 srcWidth, 
...
// only supporting HVX version in this example.
    ...
    *dspUsec = (int)(endTime - startTime);
    FARF(HIGH,"conv3x3a16_v60 profiling: %d PCycles, %d microseconds. Observed clock rate %d MHz",
