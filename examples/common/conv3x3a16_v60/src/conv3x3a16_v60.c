...
static const signed char mask[9] =
{
    1, -4, 7,
    2, -5, 8,
    3, -6, 9
...
static void conv3x3a16_ref (unsigned char *src, int srcStride, int width, int height, const signed char *mask, int shift, unsigned char *dst, int dstStride)
{
  int x, y;
  signed short sum;
  ...
  for (y = 1; y < height - 1; y++)
  {
    for (x = 1; x < width - 1; x++)
    {
        sum = src[(y-1)*srcStride + (x-1)] * mask[0]
            ...
            + src[(y)*srcStride + (x-1)] * mask[3]
            + src[(y)*srcStride + (x)] * mask[4]
            + src[(y)*srcStride + (x+1)] * mask[5]
            + src[(y+1)*srcStride + (x-1)] * mask[6]
            + src[(y+1)*srcStride + (x)] * mask[7]
...
int test_main_start(int argc, char* argv[])
{
    ...
    if (argc == 1) 
    {
        printf("Usage: conv3x3a16_v60 <width (must be multiple of 128)> <height> <number of frames> <uSec per frame> <DSP MIPS vote> <BUS MBPS vote> <verbose (0/1)>\n");
    ...
    // call API
	printf( "calling conv3x3a16_v60 on a %dx%d image...\n", (int)width, (int)height);
    ...
    for (i = 0; i < LOOPS; i++)
    {
        ...
        retVal = conv3x3a16_v60_conv3x3(src, size, stride, width, height, (const char*)mask, 9, 4, dst, size, stride, (int32*)&dspUsec);
    ...
    VERIFY(0 == retVal);
#ifdef __hexagon__
    printf("run time of conv3x3a16_v60: %llu PCycles for %d iterations\n", totalTime, LOOPS);
#else
	printf("run time of conv3x3a16_v60: %llu microseconds for %d iterations. Last iteration DSP-measured time: %d uSec\n", totalTime, LOOPS, dspUsec);
#endif
	printf("return value from conv3x3a16_v60: %d \n", retVal);

    // validate results
    conv3x3a16_ref (src, stride, width, height, mask, 4, ref, stride);
