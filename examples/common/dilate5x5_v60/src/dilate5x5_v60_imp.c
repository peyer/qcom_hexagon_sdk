...
// includes
#include "dilate5x5_v60.h"
#include "dilate5x5_asm.h"
...
static void dilate5x5_callback(
    void* data
)
{
    dilate5x5_callback_t    *dptr = (dilate5x5_callback_t*)data;
    ...
    if (tid == numThreads - 1)
    {
        length =  dptr->height - 4 - (numThreads - 1) * rowsPerJob;
    }
    ...
    unsigned char *src = dptr->src + (dptr->srcStride * (rowsPerJob * tid + 2));
    unsigned char *dst = dptr->dst + (dptr->dstStride * (rowsPerJob * tid + 2));
    unsigned char *L2FETCH_ADDR =  src + (5+AHEAD) * dptr->srcStride;
    L2fetch((unsigned int)(dptr->src + (dptr->srcStride * (rowsPerJob * tid))), CreateL2pfParam(dptr->srcStride, dptr->srcWidth, 8, 1));

    // next prefetches will just add 1 row
    long long L2FETCH_PARA = CreateL2pfParam(dptr->srcStride, dptr->srcWidth, 4, 1);
    ...
    for (i = 0; i < length; i+=4)
    {
        ...
        dilate5x5Per2Row(src, dptr->srcStride, dptr->srcWidth, dst, dptr->dstStride);
        src += 4 * dptr->srcStride;
        dst += 4 * dptr->dstStride;
        L2FETCH_ADDR += 4 * dptr->srcStride;
    }
...
AEEResult dilate5x5_v60_dilate(
    const uint8* imgSrc, 
    int inpLen, 
    int32 srcStride, 
    int32 srcWidth, 
...
// only supporting HVX version in this example.
    ...
    dptr.rowsPerJob = ((dptr.height - 4 + (numWorkers - 1)) / (numWorkers)) & -4;
    job.dptr = (void *)&dptr;
    ...
    *dspUsec = (int)(endTime - startTime);
    FARF(HIGH,"dilate5x5_v60 profiling: %d PCycles, %d microseconds. Observed clock rate %d MHz",
