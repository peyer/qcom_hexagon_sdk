...
void dilate5x5_ref (unsigned char *src, int srcStride, int width, int height, unsigned char *dst, int dstStride)
{
  ...
  for (y = 2; y < height - 2; y++)
  {
    for (x = 2; x < width - 2; x++)
    {
        max = MAX(src[(y-2)*srcStride + (x-2)], src[(y-2)*srcStride + (x-1)]);
        ...
        max = MAX(max, src[(y+0)*srcStride + (x+0)]);
        max = MAX(max, src[(y+0)*srcStride + (x+1)]);
        max = MAX(max, src[(y+0)*srcStride + (x+2)]);
        max = MAX(max, src[(y+1)*srcStride + (x-2)]);
        max = MAX(max, src[(y+1)*srcStride + (x-1)]);
...
int test_main_start(int argc, char* argv[])
{
    int WIDTH = 1920;        // smaller size on simulator for faster run time
    int HEIGHT = 1080;
    int LOOPS = 1;
    int USEC = 33333;
    int MIPS = 1000;
    ...
    if (argc == 1) 
    {
        printf("Usage: dilate5x5_v60 <width (must be multiple of 128)> <height> <number of frames> <uSec per frame> <DSP MIPS vote> <BUS MBPS vote> <MODE (0=normal, 1=CPU wakeup + FastRPC, 2=CPU wakeup only)> <verbose (0/1)>\n");
        printf("No arguments given, using defaults: %d %d %d %d %d %d %d %d.\n",WIDTH,HEIGHT,LOOPS,USEC,MIPS,MBPS,MODE,VERBOSE);
    }
    else if (argc != 9)
    {
        printf("Usage: dilate5x5_v60 <width (must be multiple of 128)> <height> <number of frames> <uSec per frame> <DSP MIPS vote> <BUS MBPS vote> <MODE (0=normal, 1=CPU wakeup + FastRPC, 2=CPU wakeup only)> <verbose (0/1)>\n");
    ...
    // call API
	printf( "calling dilate5x5_v60 on a %dx%d image...\n", (int)width, (int)height);
    ...
    for (i = 0; i < LOOPS; i++)
    {
        ...
        if (MODE < 2)
            retVal = dilate5x5_v60_dilate(src, size, stride, width, height, dst, size, stride, MODE, (int32*)&dspUsec);
    ...
    VERIFY(0 == retVal);
#ifdef __hexagon__
    printf("run time of dilate5x5_v60: %llu PCycles for %d iterations\n", totalTime, LOOPS);
#else
	printf("run time of dilate5x5_v60: %llu microseconds for %d iterations. Last iteration DSP-measured time: %d uSec\n", totalTime, LOOPS, dspUsec);
#endif
	printf("return value from dilate5x5_v60: %d \n", retVal);
/*
    // validate results
