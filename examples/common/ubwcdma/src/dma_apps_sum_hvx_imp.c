/*! *****************************************************************************
 * @file            dma_apps_sum_hvx_imp.c
 *
 * @services        dma sum hvx app implementation
 *
 ...
 *                  The core sum_hvx operation copies 2 frames into L2 in ROI
 *                  sized chunks, sums them using the hvx and writes the
 *                  final result into the provided buffers.
 *
...
typedef struct
{
    // Handle return by the core sum app
    t_HandleSum hdlSum;
    // Structures to provide to the core sum app.
    t_StDmaSum_OpenParm stOpenParm;
    t_StDmaSum_MoveParm stMoveParm;
...
AEEResult dma_apps_sum_hvx_scratch_size(int *size)
{
    *size = sizeof(t_StDmaSumAppHdl);
    DBG_LOGI("dma_apps_sum_hvx required scratch size is %d bytes\n",
             sizeof(t_StDmaSumAppHdl));
    return AEE_SUCCESS;
}

AEEResult dma_apps_sum_hvx_open(const dma_apps_cfg_t* cfg, dma_apps_hdl_t* p_hdl)
{
    ...
    t_StDmaSumAppHdl* p_app_hdl = (t_StDmaSumAppHdl*) p_hdl->app_scratch;

     // Perform some checks
    if (p_hdl->app_scratchLen != sizeof(t_StDmaSumAppHdl))
    {
    ...
    // Since this is an HVX based app, the stride must always be aligned to 128
    // bytes in the TCM. This is to ensure that each line starts at an aligned
    // address. Note: This does not affect UBWC mode since the stride is 256
    // byte aligned there but does affect linear mode. Alignment for 2 byte per
    // pixel TCM formats need only be 64 since strides are expressed in pixels.
    int tcmStrideAlignment = 64;
    // Alignment for 1 byte per pixel TCM formats is 128.
    if ((p_app_hdl->stMoveParm.eFmt == eDmaFmt_NV12 ||
         p_app_hdl->stMoveParm.eFmt == eDmaFmt_NV124R) &&
        !p_app_hdl->stMoveParm.bUse16BitPaddingInL2)
    {
        tcmStrideAlignment *= 2;
    }
    tcmStride = ALIGN(tcmStride, tcmStrideAlignment);
    ...
    // This is an application that utilizes 128 byte HVX vectors. As such we
    // add 0x7f (127) to the buffer size so that we can ensure that the
    // start addresses of the ping-pong buffers are aligned to 128 for correct
    // HVX vector processing.
    p_app_hdl->region_tcm_size = ALIGN(tcm_buf_size*6 + 0x7f, 0x1000);
    ...
    DBG_LOGE("tcm_buf_addr=0x%x\n", (unsigned) tcm_buf_paddr);
    ...
    // addresses are also needed since the TCM will be accessed to do the sum,
    // which was not the case in the memcpy.
    // 
    // Each buffer address is ensured to be aligned by 128 due to the following:
    // 1. We've over allocated p_app_hdl->region_tcm_size accordingly
    // 2. nDmaWrapper_GetRecommendedIntermBufSize will also return sizes
    //    that are 256 aligned 
    p_app_hdl->stMoveParm.addrL2VirtAddr_IntermBufPingRd =
        (p_app_hdl->tcm_buf_vaddr + 0x7f) & ~0x7f;
    ...
    // The core Sum app is invoked to initialize the dma
    p_app_hdl->hdlSum = hDmaSum_Open(p_app_hdl->stOpenParm);

    if (!p_app_hdl->hdlSum) {
        DBG_LOGE("Failed to open a session to start the sum hvx application.\n");
        goto err5;
    }
    ...
    DBG_LOGI("Successfully opened the sum application\n");
    goto done;
...
AEEResult dma_apps_sum_hvx_run(const dma_apps_hdl_t* p_hdl,
                                ...
                                unsigned char* dst, int dstLen)
{
    ...
    // Lock the HVX for the duration of the application.
    if (0 != qurt_hvx_lock(QURT_HVX_MODE_128B))
    {
        DBG_LOGE("Failed to lock the HVX. Cannot run the sum application\n");
        goto err1;
    }

    // The core Sum app is then invoked to begin the transfer.
    if (OK != nDmaSum_Move(p_app_hdl->hdlSum, p_app_hdl->stMoveParm))
    {
        DBG_LOGE("Failed to execute the sum application.\n");
        goto err2;
    }

    DBG_LOGI("Successfully executed the sum application\n");
    ret = AEE_SUCCESS;
...
AEEResult dma_apps_sum_hvx_close(const dma_apps_hdl_t* p_hdl)
{
    AEEResult ret = AEE_SUCCESS;
    t_StDmaSumAppHdl* p_app_hdl = (t_StDmaSumAppHdl*) p_hdl->app_scratch;

    if (!p_app_hdl->hdlSum || p_app_hdl->tcm_buf_vaddr == NULL ||
    ...
    // Close the Sum session.
    if (p_app_hdl->hdlSum && nDmaSum_Close(p_app_hdl->hdlSum) != OK)
    {
        DBG_LOGE("Failed to close the sum application.\n");
        ret = AEE_EFAILED;
    }
    ...
    if (ret != AEE_EFAILED)
    {
        DBG_LOGI("Successfully closed the sum application\n");
    }
