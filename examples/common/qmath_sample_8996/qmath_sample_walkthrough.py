...
example_name = "qmath_sample_8996"

target_info = target_list()
unsupported_target_info =["8998","sdm660","sdm670","sdm845"]

#******************************************************************************
# Parser for cmd line options
#******************************************************************************
parser = argparse.ArgumentParser(prog='qmath_sample_walkthrough.py', description=__doc__, formatter_class=RawTextHelpFormatter)
call_parser(parser)  
...
def run_example():
	...
	example_name_exe=HEXAGON_SDK_ROOT +EXAMPLE_PATH+example_name+'/'+variant+'/ship/qmath_sample'
#	libexample_name=HEXAGON_SDK_ROOT +EXAMPLE_PATH+example_name+'/'+variant+'/ship/lib'+example_name+'.so'
	libexample_name_skel=HEXAGON_SDK_ROOT +EXAMPLE_PATH+example_name+'/'+hex_variant+'/ship/libqmath_sample_skel.so'
	...
	if not options.no_rebuild:
		print "---- Build qmath_sample_8996 example for both Android and Hexagon ----"
		...
		print "---- Skip rebuilding qmath_sample_8996 example for both Android and Hexagon ----"
	...
	print_and_run_cmd('adb -s '+device_number+'  wait-for-device shell chmod 777 '+APPS_DST+'/qmath_sample')
	...
	print_and_run_cmd('adb -s '+device_number+'  wait-for-device shell "echo 0x1f > '+DSP_DST+'qmath_sample.farf"')
	if example_name=="cornerApp" :
		print_and_run_cmd('adb -s '+device_number+'  wait-for-device push '+libexample_name+' '+LIB_DST)
		print_and_run_cmd('adb -s '+device_number+'  wait-for-device push '+ HEXAGON_SDK_ROOT +'/libs/fastcv/fastcv/hexagon_Release_dynamic/libfastcvadsp.so '+DSP_DST)
	...
	print("---- Run qmath_sample_8996 Example on " +Flag[:4] +" ----")
	print_and_run_cmd('adb -s '+device_number+'  wait-for-device shell export LD_LIBRARY_PATH='+LIB_DST+' '+DSP_LIB_SEARCH_PATH+'='+DSP_LIB_PATH+' '+APPS_DST+'/qmath_sample')
