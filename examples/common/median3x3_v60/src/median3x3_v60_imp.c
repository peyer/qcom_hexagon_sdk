...
@file
   median3x3_v60_imp.cpp

@brief
   implementation file for median filter RPC interface.
...
} median3x3_callback_t;

/*===========================================================================
    LOCAL FUNCTION
===========================================================================*/
// multi-threading callback function
static void median3x3_callback(
    void* data
)
{
    median3x3_callback_t    *dptr = (median3x3_callback_t*)data;
    ...
    for (i = 0; i < length; i+=1)
    {
        ...
        median3x3PerRow(src, dptr->srcStride, dptr->srcWidth, dst);
        src += dptr->srcStride;
        dst += dptr->dstStride;
...
AEEResult median3x3_v60_median(
    const uint8* imgSrc, 
    int inpLen, 
    int32 srcStride, 
    int32 srcWidth, 
...
// only supporting HVX version in this example.
    ...
    *dspUsec = (int)(endTime - startTime);
    FARF(HIGH,"median3x3_v60 profiling: %d PCycles, %d microseconds. Observed clock rate %d MHz",
