...
static void median3x3_ref (unsigned char *src, int srcStride, int width, int height, unsigned char *dst)
{
  int x, y;
    ...
    for (x = 1; x < width - 1; x++)
    {
        ...
        for(n=0;n<8;n++)
        {
            for (m=8;m>n;m--)
            {
                if (p[m] > p[m-1])
                {
                    unsigned char tmp = p[m];
                    p[m] = p[m-1];
                    p[m-1] = tmp;
                }
            }
        }    
        dst[(y)*srcStride + (x)] = p[4];
    } 
  }
}
...
int test_main_start(int argc, char* argv[])
{
    ...
    if (argc == 1) 
    {
        printf("Usage: median3x3_v60 <width (must be multiple of 128)> <height> <number of frames> <uSec per frame> <DSP MIPS vote> <BUS MBPS vote> <verbose (0/1)>\n");
    ...
    // call API
	printf( "calling median3x3_v60 on a %dx%d image...\n", (int)width, (int)height);
    ...
    for (i = 0; i < LOOPS; i++)
    {
        ...
        retVal = median3x3_v60_median(src, size, stride, width, height, dst, size, (int32*)&dspUsec);
    ...
    VERIFY(0 == retVal);
#ifdef __hexagon__
    printf("run time of median3x3_v60: %llu PCycles for %d iterations\n", totalTime, LOOPS);
#else
	printf("run time of median3x3_v60: %llu microseconds for %d iterations. Last iteration DSP-measured time: %d uSec\n", totalTime, LOOPS, dspUsec);
#endif
	printf("return value from median3x3_v60: %d \n", retVal);

    // validate results
    median3x3_ref (src, stride, width, height, ref);
