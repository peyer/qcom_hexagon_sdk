...
example_name = "qfxp_sample"
target_info = target_list()
unsupported_target_info =["8996","sdm660"]
#******************************************************************************
# Parser for cmd line options
#******************************************************************************
parser = argparse.ArgumentParser(prog='qfxp_sample_walkthrough.py', description=__doc__, formatter_class=RawTextHelpFormatter)
call_parser(parser)  
...
#	libexample_name=HEXAGON_SDK_ROOT +EXAMPLE_PATH+example_name+'/'+variant+'/ship/lib'+example_name+'.so'
	...
	if not options.no_rebuild:
		print "---- Build qfxp_sample example for both Android and Hexagon ----"
		...
		print "---- Skip rebuilding qfxp_sample example for both Android and Hexagon ----"
	...
	print("---- Run qfxp_sample Example on " +Flag[:4] +" ----")
