...
AEEResult conv3x3a32_v60_conv3x3(
    const uint8* imgSrc, 
    int inpLen, 
    int32 srcStride, 
    int32 srcWidth, 
...
// only supporting HVX version in this example.
    ...
    *dspUsec = (int)(endTime - startTime);
    FARF(HIGH,"conv3x3a32_v60 profiling: %d PCycles, %d microseconds. Observed clock rate %d MHz",
