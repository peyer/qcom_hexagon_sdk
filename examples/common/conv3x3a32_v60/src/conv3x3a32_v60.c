...
static const signed char mask[12] =
{
    1, -4, 7, 0,
    2, -5, 8, 0,
    3, -6, 9, 0
...
static void conv3x3a32_ref (unsigned char *src, int srcStride, int width, int height, const signed char *mask, int shift, unsigned char *dst, int dstStride)
{
  int x, y;
  int sum;
...
int test_main_start(int argc, char* argv[])
{
    ...
    if (argc == 1) 
    {
        printf("Usage: conv3x3a32_v60 <width (must be multiple of 128)> <height> <number of frames> <uSec per frame> <DSP MIPS vote> <BUS MBPS vote> <verbose (0/1)>\n");
    ...
    // call API
	printf( "calling conv3x3a32_v60 on a %dx%d image...\n", (int)width, (int)height);
    ...
    for (i = 0; i < LOOPS; i++)
    {
        ...
        retVal = conv3x3a32_v60_conv3x3(src, size, stride, width, height, (const char*)mask, 12, 4, dst, size, stride, (int32*)&dspUsec);
    ...
    VERIFY(0 == retVal);
#ifdef __hexagon__
    printf("run time of conv3x3a32_v60: %llu PCycles for %d iterations\n", totalTime, LOOPS);
#else
	printf("run time of conv3x3a32_v60: %llu microseconds for %d iterations. Last iteration DSP-measured time: %d uSec\n", totalTime, LOOPS, dspUsec);
#endif
	printf("return value from conv3x3a32_v60: %d \n", retVal);

    // validate results
    conv3x3a32_ref (src, stride, width, height, mask, 4, ref, stride);
