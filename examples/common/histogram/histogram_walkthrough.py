...
#******************************************************************************
parser = argparse.ArgumentParser(prog='histogram_walkthrough.py', description=__doc__, formatter_class=RawTextHelpFormatter)
call_parser(parser) 
...
#	libexample_name=HEXAGON_SDK_ROOT +EXAMPLE_PATH+example_name+'/'+variant+'/ship/lib'+example_name+'.so'
	...
	if not options.no_rebuild:
		print "---- Build histogram example for both Android and Hexagon ----"
		...
		print "---- Skip rebuilding histogram example for both Android and Hexagon ----"
	...
	print("---- Run histogram Example on " +Flag[:4] +" ----")
