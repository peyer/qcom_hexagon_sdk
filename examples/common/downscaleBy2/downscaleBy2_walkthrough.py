...
example_name = "downscaleBy2"
target_info = target_list()
unsupported_target_info = []
#******************************************************************************
# Parser for cmd line options
#******************************************************************************
parser = argparse.ArgumentParser(prog='downscaleBy2_walkthrough.py', description=__doc__, formatter_class=RawTextHelpFormatter)
call_parser(parser)
...
#	libexample_name=HEXAGON_SDK_ROOT +EXAMPLE_PATH+example_name+'/'+variant+'/ship/lib'+example_name+'.so'
	...
	if not options.no_rebuild:
		print "---- Build downscaleBy2 example for both Android and Hexagon ----"
		...
		print "---- Skip rebuilding downscaleBy2 example for both Android and Hexagon ----"
	...
	print("---- Run downscaleBy2 Example on " +Flag[:4] +" ----")
