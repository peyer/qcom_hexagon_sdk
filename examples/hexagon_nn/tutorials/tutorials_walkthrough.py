...
tutorials_list = ["001-nop", "002-add", "003-quantized-add", "004-xor-graph"]

example_name = "tutorials"

#******************************************************************************
# Parser for cmd line options
#******************************************************************************
parser = argparse.ArgumentParser(prog='tutorials_walkthrough.py', description=__doc__, formatter_class=RawTextHelpFormatter)
call_parser(parser)
...
# run walkthrough
def run_example():
	if not os.getenv('HEXAGON_NN'):
		sys.exit("\nHEXAGON_NN not set up -> please run setup_hexagon_nn script from SDK's examples/hexagon_nn directory\n")
	...
	EXAMPLE_PATH='/examples/hexagon_nn/'
	...
	libexample_name_skel=HEXAGON_NN+'/'+hex_variant+'/ship/libhexagon_nn_skel.so'
	...
	if not options.no_rebuild:
		...
		print "---- Skip rebuilding Hexagon_NN example for both Android and Hexagon ----"
	...
	print ("---- Run Examples on cDSP ----")
	for tutorial in tutorials_list:
		print ("---- Runing "+tutorial+"		----")
		example_name_exe=HEXAGON_SDK_ROOT +EXAMPLE_PATH+example_name+'/'+variant+'/ship/'+tutorial
		print_and_run_cmd('adb -s '+device_number+'  wait-for-device push '+example_name_exe+' '+APPS_DST)
		print_and_run_cmd('adb -s '+device_number+'  wait-for-device shell chmod 777 '+APPS_DST+'/'+tutorial)
		print_and_run_cmd('adb -s '+device_number+'  wait-for-device shell export LD_LIBRARY_PATH='+LIB_DST+' '+DSP_LIB_SEARCH_PATH+'='+DSP_LIB_PATH+' '+APPS_DST+'/'+tutorial)
