...
uint32_t calculateNumBuffers(const capi_v2_buf_t* format)
{
  ...
  QURT_ELITE_ASSERT(format->actual_data_len >= sizeof(tst_standard_media_format));

  tst_standard_media_format* tst_format = (tst_standard_media_format*)format->data_ptr;
...
static capi_v2_err_t update_output_media_format(module_info_t* module, const capi_v2_buf_t* out_format)
{
  QURT_ELITE_ASSERT(out_format->actual_data_len <= sizeof(module->out_format_buf));
  QURT_ELITE_ASSERT(out_format->actual_data_len <= module->out_format.max_data_len);
  ...
  memcpy(module->out_format.data_ptr, out_format->data_ptr, out_format->actual_data_len);
  ...
  if (IsCompressedData(out_format)) {
    ...
    module->out_buffer_len = (format->f.sampling_rate / 1000) * bits_to_bytes(format->f.bits_per_sample);
  }
  return CAPI_V2_EOK;
}
...
capi_v2_err_t FillInputBuffer(capi_v2_stream_data_t* input_bufs_ptr, module_info_t* module, void* temp_in_buffer)
{
  ...
  if (num_bytes_read < num_bytes_to_read) {
	FARF(HIGH, "CAPI V2 TEST: Input file seems to be complete");
  }
  ...
  if (IsCompressedData(&module->in_format)) {
    ...
    QURT_ELITE_ASSERT(module->in_format.actual_data_len >= sizeof(tst_standard_media_format));
    tst_standard_media_format* format = (tst_standard_media_format*)module->in_format.data_ptr;
...
capi_v2_err_t DumpOutputToFile(capi_v2_stream_data_t* output_bufs_ptr, module_info_t* module, void* temp_out_buffer)
{
  ...
  if (IsCompressedData(&module->out_format)) {
    ...
    QURT_ELITE_ASSERT(module->in_format.actual_data_len >= sizeof(tst_standard_media_format));
    // JV-???: Shouldn't this be the output format?
//		tst_standard_media_format *format = reinterpret_cast<tst_standard_media_format*>(module->in_format.data_ptr);
    tst_standard_media_format* format = (tst_standard_media_format*)module->out_format.data_ptr;
...
capi_v2_err_t GetIntParameter(FILE* fCfg, const char* param_name, int32_t* pValue)
{
  ...
  if (!test_script) {
    ...
    if (0 != strncmp(word, param_name, WORDSIZE)) {
      FARF(ERROR, "CAPI V2 TEST: Did not find the %s parameter",
           param_name);
      return CAPI_V2_EFAILED;
    }
  }
...
capi_v2_err_t ProcessData(module_info_t* module)
{
  ...
  capi_v2_buf_t in[CAPI_V2_MAX_CHANNELS], out[CAPI_V2_MAX_CHANNELS];
  ...
  for (i = 0; i < numBuffers; i++) {
    ...
    while (input_str_data[0].buf_ptr[0].actual_data_len > 0) {
      ...
      if ( module->fout != NULL &&  !IsCompressedData(&module->out_format)) {
        ...
        tst_standard_media_format* tst_format = (tst_standard_media_format*)module->out_format.data_ptr;
  ...
  if (!IsCompressedData(&module->out_format)) {
    ...
    #if FARF_HIGH == 1
    tst_standard_media_format* tst_format;
    if (module->fout != NULL) {
      tst_format = (tst_standard_media_format*)module->out_format.data_ptr;
    } else {
      tst_format = (tst_standard_media_format*)module->in_format.data_ptr;
    }
    #endif
    FARF(HIGH, "-------------------------------");
...
capi_v2_err_t ParseMediaFormat(module_info_t* module, tst_standard_media_format* format)
{
  capi_v2_err_t result = CAPI_V2_EOK;
...
capi_v2_err_t SetOutputMediaFormat(module_info_t* module)
{
  ...
  tst_standard_media_format out_format = { };
  ...
  if (IsCompressedData(&module->out_format)) {
    ...
    module->out_buffer_len = (out_format.f.sampling_rate / 1000) * bits_to_bytes(out_format.f.bits_per_sample);
  }
  ...
  // ------------------------
  {
    ...
    if (CAPI_V2_EOK != result) {
      FARF(ERROR, "CAPI V2 TEST: Cannot set output media format, result=%d", result);
    }
  }
...
capi_v2_err_t SetMediaFormat(module_info_t* module)
{
  ...
  tst_standard_media_format in_format = { };
  ...
  if (IsCompressedData(&module->in_format)) {
    ...
    module->in_buffer_len = (in_format.f.sampling_rate / 1000) * bits_to_bytes(in_format.f.bits_per_sample);
  }
  ...
  switch(id) {
    ...
    case CAPI_V2_EVENT_BANDWIDTH:
    {
      if (payload->actual_data_len < sizeof(capi_v2_event_bandwidth_t)) {
        FARF(ERROR, "CAPI V2 TEST: Error in callback function. The actual size %lu is less than the required size %zu for id %lu.", payload->actual_data_len, sizeof(capi_v2_event_bandwidth_t), (uint32_t)id);
        return CAPI_V2_ENEEDMORE;
      }

      module->code_bw = ((capi_v2_event_bandwidth_t*)(payload->data_ptr))->code_bandwidth;
      module->data_bw = ((capi_v2_event_bandwidth_t*)(payload->data_ptr))->data_bandwidth;
      FARF(HIGH, "CAPI V2 TEST: Module Code BW set to %lu", (uint32_t)module->code_bw);
      FARF(HIGH, "CAPI V2 TEST: Module Data BW set to %lu", (uint32_t)module->data_bw);
    ...
    case CAPI_V2_EVENT_OUTPUT_MEDIA_FORMAT_UPDATED:
    {
      ...
      if (event_ptr->port_info.is_input_port) {
        FARF(ERROR, "CAPI V2 TEST: Error in callback function. Changing media type only supported for output ports. Input port id passed.");
        return CAPI_V2_EUNSUPPORTED;
      }
      ...
      if (CAPI_V2_EOK != update_output_media_format(module, payload)) {
        return CAPI_V2_EFAILED;
      }
