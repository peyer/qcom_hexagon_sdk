/*
 * Copyright (c) 2013-2015, 2021 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
static int rpcmem_ionfd = -1;
static int rpcmem_flag;
static int rpcmem_ionversion = RPCION_VERSION_UNKNOWN;

static int rpcmem_init_counter = 0;
static int rpcmem_init_once_ret_val = -1;
...
static void rpcmem_init_once() {
   int nErr = AEE_SUCCESS;

   nErr = pthread_mutex_init(&mt, 0);
   if(nErr) {
      VERIFY_EPRINTF("Error 0x%x: %s failed to initialize mutex!\n", nErr, __func__);
   } else {
      QList_Ctor(&lst);
   }
   rpcmem_init_once_ret_val = nErr;
}

/*
 * When multiple threads call rpcmem_init, only one thread will call rpcmem_init_once once.
 * All other threads will wait until this call is finished. Then the first thread to acquire
 * the mutex will try to open the ionfd. If successful, all other threads will lock and unlock
 * mutex and return. If first thread failed to open ionfd, then the following threads will
 * attempt to open ionfd
 */
void rpcmem_init() {
   static pthread_once_t  rpcmem_once_control = PTHREAD_ONCE_INIT;
   int nErr = AEE_SUCCESS;

   VERIFY(AEE_SUCCESS == (nErr = pthread_once(&rpcmem_once_control, (void*)rpcmem_init_once)));
   if(rpcmem_init_once_ret_val) {
      nErr = rpcmem_init_once_ret_val;
      goto bail;
   }
   ...
   if (rpcmem_ionfd < 0) {
      nErr = errno;
      VERIFY_EPRINTF("Error: %s: ion_open failed, errno %s (init count: %d) !\n", __func__, strerror(errno), rpcmem_init_counter);
      goto bail;
   }
   rpcmem_ionversion = get_ionversion(rpcmem_ionfd);

bail:
   if(rpcmem_init_once_ret_val == AEE_SUCCESS)
      pthread_mutex_unlock(&mt);
   if (nErr)
      VERIFY_EPRINTF("Error 0x%x: %s failed\n", nErr, __func__);
}
...
void rpcmem_deinit() {
   ...
   pthread_mutex_lock(&mt);
   if (rpcmem_init_counter)
      rpcmem_init_counter--;

   // Only the last thread calling rpcmem_deinit (rpcmem_init_counter == 0) should free resources
   if(rpcmem_init_counter > 0) {
      pthread_mutex_unlock(&mt);
      return;
   }
   if (rpcmem_ionfd >= 0) {
      nErr = close(rpcmem_ionfd);
      if(nErr)
         VERIFY_EPRINTF("Error: %s: ion_close on fd %d failed, errno %s\n", __func__, rpcmem_ionfd, strerror(errno));
      rpcmem_ionfd = -1;
   }
   pthread_mutex_unlock(&mt);
}
...
void* rpcmem_alloc_internal(int heapid, uint32 flags, int size) {
   ...
   if(rpc_flags & RPCMEM_HEAP_DEFAULT || 
	heapid == RPCMEM_DEFAULT_HEAP) {
      VERIFY_IPRINTF("rpcmem: use of RPCMEM_HEAP_DEFAULT/RPCMEM_DEFAULT_HEAP is deprecated,\n\t please use RPCMEM_HEAP_ID_SYSTEM for SMMU memory and RPCMEM_HEAP_ID_CONTIG for CMA memory\n");
   }
