...
# Test on the Device
    ...
    adb push ${ANDROID_BUILD}/librpcmem.so /vendor/lib64/
    adb push ${HEXAGON_BUILD}/librpcmem_skel.so /vendor/lib/rfsa/adsp/
...
then
    ...
    if [ "$2" = "qurt" ];        
    then
        doHexagon hexagon_ReleaseG_dynamic_toolv83_v66 rpcmem_skel qurt    
    else
        doHexagon hexagon_ReleaseG_dynamic_toolv83_v66 rpcmem_skel
    fi
    cd ${_src} && \
       ${CMAKE} \
       --build \
...
then
    ...
    echo "No Simulator Test, Nothing to run" 
fi
