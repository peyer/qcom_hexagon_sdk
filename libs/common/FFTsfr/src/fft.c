...
cfloat conjugate(cfloat x)
{
  cfloat y;

  y = __real__(x) - 1i * __imag__(x);
...
// Function: N-point real-valued FFT
void FFTsfr(float* input, int N, cfloat* Wt1, cfloat* Wt2, cfloat* output)
{
  int i;
  cfloat X, Y;
  cfloat* Z = output;
  ...
  for (i = 1; i <= N / 4; i++) {
    X = Z[i] + conjugate(Z[N / 2 - i]);
    Y = Z[i] - conjugate(Z[N / 2 - i]);
