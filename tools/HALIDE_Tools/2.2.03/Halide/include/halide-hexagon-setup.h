...
void setupHexagonTarget(Target &target, Target::Feature F=Target::HVX_128) {
        target.os = Target::NoOS; // The operating system
...
void set_min(ImageParam &I, int dimen, Expr a) {
  I.dim(dimen).set_min(a);
}
void set_output_buffer_min(Func &f, int dimen, Expr a) {
  f.output_buffer().dim(dimen).set_min(a);
}
void set_stride_multiple(OutputImageParam I, int dimen, int m) {
  I.dim(dimen).set_stride((I.dim(dimen).stride() / m) * m);
}
