...
#include "dma_nv12_rw_async_halide.h"
...
#endif
// API to power on HVX
int dma_nv12_rw_async_power_on_hvx(void) {
    FARF(HIGH, "\npower_on\n");
    return power_on_hvx();
}
// API to power off HVX
int dma_nv12_rw_async_power_off_hvx(void) {
    FARF(HIGH, "\npower_off\n");
    return power_off_hvx();
}
// APIs to select HVX performance modes(Low/nomial/turbo)
int dma_nv12_rw_async_set_hvx_perf_mode_low(void) {
    FARF(HIGH, "\nperf_low\n");
    return set_hvx_perf_mode_low();
}
int dma_nv12_rw_async_set_hvx_perf_mode_nominal(void) {
    FARF(HIGH, "\nperf_nominal\n");
    return set_hvx_perf_mode_nominal();
}
int dma_nv12_rw_async_set_hvx_perf_mode_turbo(void) {
    FARF(HIGH, "\nperf_turbo\n");
    return set_hvx_perf_mode_turbo();
}
// This function Wraps the ION buffers to halide_buffer_t and invoke the Halide Pipeline code
int dma_nv12_rw_async_run(const unsigned char *input_buffer,
    ...
    nRet += halide_hexagon_dma_allocate_engine(nullptr, &dma_engine_write);
    if (nRet) {
    ...
    if (nRet) {
        ...
        halide_hexagon_dma_deallocate_engine(0, dma_engine_write);
        return nRet;
    }
    ...
    // Initialize DMA with output UV Frame details
    ...
    nRet = halide_hexagon_dma_prepare_for_copy_to_device(0, &output_y, dma_engine_write, 0, linear_format);
    if (nRet) {
        // De-allocate the Allocated DMA engine if the API fails
    ...
    nRet = halide_hexagon_dma_prepare_for_copy_to_device(0, &output_uv, dma_engine_write, 0, chroma_format);
    if (nRet) {
        // De-allocate the Allocated DMA engine if the API fails
        FARF(HIGH, "halide_hexagon_dma_prepare_for_copy_to_device returns %d \n", nRet);
        halide_hexagon_dma_deallocate_engine(0, dma_engine);
    ...
    (*avg_time) = benchmark(iterations, [&]() {
        result = dma_nv12_rw_async_halide(&input_y, &input_uv, &output_y, &output_uv);
        if (result != 0) {
            FARF(HIGH, "dma_nv12_rw_async_test failed! %d\n", result);
        }
    });
    ...
    nRet += halide_hexagon_dma_deallocate_engine(0, dma_engine_write);
    if (nRet) {
