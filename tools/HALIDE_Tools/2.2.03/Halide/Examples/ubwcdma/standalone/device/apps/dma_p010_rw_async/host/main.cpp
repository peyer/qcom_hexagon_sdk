...
int main(int argc, char *argv[]) {
    ...
    int result = dma_p010_rw_async_run(input.get_buffer(), input.len(), width, height, stride, format, is_ubwc, output.get_buffer(), output.len(), iterations, &avg_time); 
    if (result) {
    ...
    if (!verify(input_validate, output, width, h-ubwc_padding)) {
        TestReport tr("dma_p010_rw_async",avg_time, "microseconds", Mode::Device_Standalone, Result::Fail);
        tr.print(); 
        printf("Failed\n");
        exit(1);
    } else {
