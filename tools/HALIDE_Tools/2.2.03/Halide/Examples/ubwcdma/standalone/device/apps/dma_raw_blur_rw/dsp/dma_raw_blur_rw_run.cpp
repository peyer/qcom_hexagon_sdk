...
#include "dma_raw_blur_rw_halide.h"
#include "dma_def.h"
#include "dma_types.h"
...
#endif
// API to power on HVX
int dma_raw_blur_rw_power_on_hvx(void) {
    FARF(HIGH, "\npower_on\n");
    return power_on_hvx();
}
// API to power off HVX
int dma_raw_blur_rw_power_off_hvx(void) {
    FARF(HIGH, "\npower_off\n");
    return power_off_hvx();
}
// APIs to select HVX performance modes(Low/nomial/turbo)
int dma_raw_blur_rw_set_hvx_perf_mode_low(void) {
    FARF(HIGH, "\nperf_low\n");
    return set_hvx_perf_mode_low();
}
int dma_raw_blur_rw_set_hvx_perf_mode_nominal(void) {
    FARF(HIGH, "\nperf_nominal\n");
    return set_hvx_perf_mode_nominal();
}
int dma_raw_blur_rw_set_hvx_perf_mode_turbo(void) {
    FARF(HIGH, "\nperf_turbo\n");
    return set_hvx_perf_mode_turbo();
}
// This function Wraps the ION buffers to halide_buffer_t and invoke the Halide Pipeline code
int dma_raw_blur_rw_run(const unsigned char *input_buffer,
    ...
    (*avg_time) = benchmark(iterations, [&]() {
        result = dma_raw_blur_rw_halide(&input, &output);
        if (result != 0) {
            FARF(HIGH, "dma_raw_blur_rw_test failed! %d\n", result);
        }
    });
