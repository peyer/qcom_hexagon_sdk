...
#include "dma_raw_rw_async.h"
#include "dma_raw_rw_async_halide.h"
#include "dma_def.h"
#include "dma_types.h"
...
#endif
// API to power on HVX
int dma_raw_rw_async_power_on_hvx(void) {
    FARF(HIGH, "\npower_on\n");
    return power_on_hvx();
}
// API to power off HVX
int dma_raw_rw_async_power_off_hvx(void) {
    FARF(HIGH, "\npower_off\n");
    return power_off_hvx();
}
// APIs to select HVX performance modes(Low/nomial/turbo)
int dma_raw_rw_async_set_hvx_perf_mode_low(void) {
    FARF(HIGH, "\nperf_low\n");
    return set_hvx_perf_mode_low();
}
int dma_raw_rw_async_set_hvx_perf_mode_nominal(void) {
    FARF(HIGH, "\nperf_nominal\n");
    return set_hvx_perf_mode_nominal();
}
int dma_raw_rw_async_set_hvx_perf_mode_turbo(void) {
    FARF(HIGH, "\nperf_turbo\n");
    return set_hvx_perf_mode_turbo();
}
// This function Wraps the ION buffers to halide_buffer_t and invoke the Halide Pipeline code
int dma_raw_rw_async_run(const unsigned char *input_buffer,
    ...
    nRet += halide_hexagon_dma_allocate_engine(0, &dma_engine_write);
    if (nRet) {
    ...
    // Initialize DMA with Output Frame det
    ...
    nRet = halide_hexagon_dma_prepare_for_copy_to_device(0, &output, dma_engine_write, false, halide_hexagon_fmt_RawData);
    if (nRet) {
        // De-allocate the Allocated DMA engine if the API fails
        FARF(HIGH, "halide_hexagon_dma_prepare_for_copy_to_host returns %d \n", nRet);
        halide_hexagon_dma_deallocate_engine(0, dma_engine);
    ...
    // Invoke the Halide Pipeline code and time the code for benchmarking purpos
    int result = 0;
    (*avg_time) = benchmark(iterations, [&]() {
        result = dma_raw_rw_async_halide(&input, &output);
        if (result != 0) {
            FARF(HIGH, "dma_raw_rw_async_test failed! %d\n", result);
        }
    });
