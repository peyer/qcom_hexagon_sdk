...
int dma_p010_rw_async_run(const unsigned char *input_buffer,
            ...
            int stride, int format, short int is_ubwc,
            uint8_t *output_buffer, int output_bufferLen, int iterations, uint64_t *avg_time) {
    FARF(HIGH, "\ndma_p010_rw_async_run\n");
    ...
    (*avg_time) = benchmark(iterations, [&]() {
        ...
        if (result != 0) {
            FARF(HIGH, "dma_p010_rw_async_halide failed! %d\n", result);
        }
    });
