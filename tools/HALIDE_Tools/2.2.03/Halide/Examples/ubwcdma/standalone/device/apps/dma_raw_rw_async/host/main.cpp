...
int main(int argc, char *argv[]) {
    ...
    int set_perf_mode_turbo = dma_raw_rw_async_set_hvx_perf_mode_turbo();
    if (set_perf_mode_turbo != 0) {
        printf ("Error: Couldn't set perf mode to turbo: %d\n", set_perf_mode_turbo);
    }
    // Power on HVX
    int power_on = dma_raw_rw_async_power_on_hvx();
    if (power_on != 0) {
    ...
    printf("Running pipeline\n");
    int result = dma_raw_rw_async_run(input.get_buffer(), input.len(), width, height, stride, format, is_ubwc, output.get_buffer(), output.len(), iterations, &avg_time); 
    if (result) {
        printf ("Error: HVX pipeline failed with error %d\n", result);
        exit(1);
    }
    // Power of HVX as the processing is done
    dma_raw_rw_async_power_off_hvx();

    if (!verify(input_validate, output, width, h)) {
        TestReport tr("dma_raw_rw_async", avg_time, "microseconds", Mode::Device_Standalone, Result::Fail);
        tr.print(); 
        printf("Failed\n");
        exit(1);
    } else {
        TestReport tr("dma_raw_rw_async", avg_time, "microseconds", Mode::Device_Standalone, Result::Pass);
        tr.print();
    } 
