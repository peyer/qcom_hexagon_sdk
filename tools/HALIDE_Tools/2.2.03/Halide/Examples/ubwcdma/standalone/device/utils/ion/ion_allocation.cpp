...
void* alloc_ion(size_t size) {
  ...
  const int ion_flags = ion_flag_cached;
    ...
    if (handle < 0) {
        __android_log_print(ANDROID_LOG_ERROR, "pranavdb", "ion_alloc(%d, %d, %d, %d, %d) failed",
    ...
    if (buf == MAP_FAILED) {
        __android_log_print(ANDROID_LOG_ERROR, "pranavdb", "mmap(NULL, %d, PROT_READ | PROT_WRITE, MAP_SHARED, %d, 0) failed",
                            size, buf_fd);
        close(buf_fd);
        if (!use_newer_ioctl) {
            ion_free(ion_fd, handle);
...
void alloc_ion_free(void *ptr) {
   ...
   __android_log_print(ANDROID_LOG_INFO, "pranavdb", "about to free");
    free(rec);
    __android_log_print(ANDROID_LOG_INFO, "pranavdb", "free succeeded");
}
