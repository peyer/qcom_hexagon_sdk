...
#include "hvx.cfg.h"
  ...
  #include "median_hvx128.h"
  #include "median_hvx64.h"
#else
  extern "C" {
    #include "median.h"
  }
...
int main(int argc, char **argv) {
  ...
  char *filename = (char *) malloc(100 * sizeof(char));
    ...
    cycles_hvx64 = benchmark([&]() {
        int error = median_hvx64(input1_buf, output_buf);
        if (error != 0) {
          printf("median_hvx64 pipeline failed: %d\n", error);
        }
      });
    SIM_RELEASE_HVX;

    printf("AppReported (HVX64B-mode): Image %dx%d - median(64B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_hvx64/width/height);
    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "median_hvx64_");
    ...
    cycles_hvx128 = benchmark([&]() {
        int error = median_hvx128(input1_buf, output_buf);
        if (error != 0) {
          printf("median_hvx128 pipeline failed: %d\n", error);
        }
      });
    SIM_RELEASE_HVX;

    printf("AppReported (HVX128B-mode): Image %dx%d - median(128B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_hvx128/width/height);
    strcpy(filename, "median_hvx128_");
    strcat(filename, argv[4]);
    ...
    cycles_basline_hvx64 = benchmark([&]() {
        int error = median_hvx64_i(input, stride, width, height, output);
        if (error != 0) {
          printf("median_baseline_hvx64 pipeline failed: %d\n", error);
        }
      });

    SIM_RELEASE_HVX;
    printf("AppReported (HVX64B-mode): Image %dx%d - median_basline(64B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_basline_hvx64/width/height);

    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "median_baseline_hvx64_");
    strcat(filename, argv[4]);
    ...
    cycles_basline_hvx128 = benchmark([&]() {
        int error = median_hvx128_i(input, stride, width, height, output);
        if (error != 0) {
          printf("median_baseline_hvx128 pipeline failed: %d\n", error);
        }
      });

    SIM_RELEASE_HVX;
    printf("AppReported (HVX128B-mode): Image %dx%d - median_basline(128B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_basline_hvx128/width/height);

    /* -----------------------------------------------------*/
    /*  Write image output to file                          */
    /* -----------------------------------------------------*/
    strcpy(filename, "median_baseline_hvx128_");
    strcat(filename, argv[4]);
